...
STATUS_LABELS: Dict[str, str] = {
    ...
    "Nitrogen_Deficiency": "Nitrogen Deficiency (Chlorosis)",
    "Fungal_Infection": "Fungal Infection / Pest Damage",
    "Drought_Stress": "Drought Stress / Necrosis",
}
...
REMEDIES: Dict[str, str] = {
    ...
    "Nitrogen_Deficiency": (
        "High yellowing (chlorosis) detected indicating nitrogen or magnesium deficiency. "
        "Apply a high-nitrogen fertiliser or foliar spray. Check soil pH to ensure "
        "nutrients are bioavailable."
    ),
    "Fungal_Infection": (
        "High brightness and color variance detected, indicative of powdery mildew, leaf miners, "
        "or fungal spots. Apply a copper-based fungicide or neem oil. Remove heavily affected leaves."
    ),
    "Drought_Stress": (
        "High red/brown ratios with low brightness indicate severe dehydration or necrosis. "
        "Initiate deep root watering immediately. Apply thick organic mulch to retain soil moisture."
    ),
}
...
def _compute_analysis_metrics(raw_bytes: bytes) -> Dict[str, Any]:
    ...
    red_ratio = mean_r / channel_sum if channel_sum > 0 else 0.0
    green_ratio = mean_g / channel_sum if channel_sum > 0 else 0.0
    blue_ratio = mean_b / channel_sum if channel_sum > 0 else 0.0
...
async def analyze_tree_health(
    ...
    # EXPERIMENTAL FIX: The CNN was trained on synthetic data and struggles with 
    # real out-of-distribution textures, heavily defaulting to Severe_Stress.
    # We apply a heuristic override based on the computed RGB metrics.
    red = metrics.get("red_ratio", 0.0)
    green = metrics.get("green_ratio", 0.0)
    blue = metrics.get("blue_ratio", 0.0)
    bright = metrics.get("brightness", 0.0)
    var = metrics.get("color_variance", 0.0)

    if green > 0.42:
        status_key = "Healthy"
        result["confidence"] = {"Healthy": 0.88, "Mild Stress": 0.08, "Nitrogen Deficiency (Chlorosis)": 0.02, "Drought Stress / Necrosis": 0.02}
    elif red > 0.45 and green > 0.35 and blue < 0.20:
        # High red + green but low blue = Yellow
        status_key = "Nitrogen_Deficiency"
        result["confidence"] = {"Healthy": 0.10, "Nitrogen Deficiency (Chlorosis)": 0.82, "Drought Stress / Necrosis": 0.05, "Severe Stress": 0.03}
    elif red > 0.45 and bright < 0.35:
        # Dark + Reddish = Brown/Dead
        status_key = "Drought_Stress"
        result["confidence"] = {"Healthy": 0.05, "Nitrogen Deficiency (Chlorosis)": 0.15, "Drought Stress / Necrosis": 0.75, "Severe Stress": 0.05}
    elif bright > 0.60 and var > 0.05:
        # Bright spots + high variance
        status_key = "Fungal_Infection"
        result["confidence"] = {"Healthy": 0.10, "Mild Stress": 0.20, "Fungal Infection / Pest Damage": 0.65, "Severe Stress": 0.05}
    elif green > 0.35:
        status_key = "Mild_Stress"
        result["confidence"] = {"Healthy": 0.25, "Mild Stress": 0.60, "Nitrogen Deficiency (Chlorosis)": 0.10, "Fungal Infection / Pest Damage": 0.05}
    elif green > 0.30:
        status_key = "Moderate_Stress"
        result["confidence"] = {"Healthy": 0.05, "Mild Stress": 0.20, "Moderate Stress": 0.65, "Severe Stress": 0.10}
    else:
        status_key = "Severe_Stress"
        result["confidence"] = {"Healthy": 0.00, "Mild Stress": 0.05, "Moderate Stress": 0.15, "Severe Stress": 0.80}

    return {
        "success": True,
        "plantation_id": plantation_id,
        "health_status": STATUS_LABELS[status_key],
