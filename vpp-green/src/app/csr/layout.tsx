...
import { Leaf, LayoutDashboard, Target, Briefcase, FileText, Settings, LogOut, Trophy } from "lucide-react";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { ProtectedRoute } from "@/components/auth/ProtectedRoute";
import { useAuthStore } from "@/lib/auth-store";

const navItems = [
  { name: "Dashboard", href: "/csr/dashboard", icon: LayoutDashboard },
  { name: "Leaderboard", href: "/csr/leaderboard", icon: Trophy },
  { name: "Sponsorships", href: "/csr/sponsorships", icon: Target },
  { name: "Projects", href: "/csr/projects", icon: Briefcase },
  { name: "Impact Reports", href: "/csr/reports", icon: FileText },
  { name: "Settings", href: "/csr/settings", icon: Settings },
];
...
export default function CsrLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <ProtectedRoute allowedRoles={["CSR_PARTNER", "CSR_USER"]}>
      <div className="min-h-screen bg-muted/20 flex">
        <aside className="w-72 bg-card/80 backdrop-blur-xl border-r border-border hidden lg:flex flex-col relative z-20">
          <div className="h-16 flex items-center px-6 border-b border-border/50 gap-2">
            ...
            <span className="font-heading font-bold text-lg text-primary">CSR Sponsor Portal</span>
          </div>
          
          <div className="p-6">
            <div className="flex items-center gap-4 mb-8">
              <Avatar className="h-12 w-12 border-2 border-primary/20">
                <AvatarFallback>TATA</AvatarFallback>
              </Avatar>
              <div>
                <p className="font-medium text-sm text-foreground">{user?.name || 'User'}</p>
                <p className="text-xs text-muted-foreground">{user?.role || 'CSR_PARTNER'}</p>
              </div>
            </div>
