...
import { Leaf, LayoutDashboard, Users, GraduationCap, BarChart3, FileText, Settings, LogOut, Trophy, Calendar } from "lucide-react";
...
const navItems = [
  ...
  { name: "Leaderboard", href: "/institution/leaderboard", icon: Trophy },
  { name: "Events & Drives", href: "/institution/events", icon: Calendar },
  { name: "Departments", href: "/institution/departments", icon: GraduationCap },
  { name: "Department HODs", href: "/institution/hods", icon: Users },
  { name: "Faculty", href: "/institution/faculty", icon: Users },
  { name: "Students", href: "/institution/students", icon: Users },
  { name: "Verifications", href: "/institution/verifications", icon: FileText },
  { name: "Campaigns", href: "/institution/campaigns", icon: BarChart3 },
...
export default function InstitutionLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <ProtectedRoute allowedRoles={["HOD", "INSTITUTION_ADMIN"]}>
      <div className="min-h-screen bg-muted/20 flex">
        ...
        <aside className="w-72 bg-card/80 backdrop-blur-xl border-r border-border hidden lg:flex flex-col relative z-20">
          <div className="h-16 flex items-center px-6 border-b border-border/50 gap-2">
            ...
            <span className="font-heading font-bold text-lg text-primary">Institution Portal</span>
          </div>
          
          <div className="p-6">
            <div className="flex items-center gap-4 mb-8">
              <Avatar className="h-12 w-12 border-2 border-primary/20">
                <AvatarImage src="https://images.unsplash.com/photo-1562774053-701939374585?auto=format&fit=crop&q=80&w=150&h=150" />
                <AvatarFallback>PU</AvatarFallback>
              </Avatar>
              <div>
                <p className="font-medium text-sm text-foreground">{user?.name || 'User'}</p>
                <p className="text-xs text-muted-foreground">{user?.role || 'INSTITUTION_ADMIN'}</p>
              </div>
            </div>
        ...
        <main className="flex-1 flex flex-col h-screen overflow-hidden relative">
          <div className="absolute top-0 right-0 w-[600px] h-[600px] bg-secondary/5 rounded-full blur-[100px] -z-10 pointer-events-none" />
